...
impl Patchable for LoginUser {}

pub async fn create_user(db: Data<SurrealDBRepo>, data: UserEntry) -> Result<bool, Error>{
    let sql = format!("SELECT * FROM user WHERE name CONTAINS '{}' LIMIT 1", data.name); 
...
pub async fn login_user(db: Data<SurrealDBRepo>, data: LoginUser) -> Result<Object, Error>{
